...
playerDisconnect() // Called when player disconnect from server
{
	...
	if( level.dvar["show_guids"] )
		iPrintln( self.name + " ^2" + "^7left the game" );
	else
...
//	self endon( "disconnect" );
	...
	if(isdefined(self.savedposition) && isdefined(self.savedangles))
		{
			org = self.savedposition;
			angles = self.savedangles;
			self spawn(  org, angles );
		}

	if(!isdefined(self.savedposition) && !isdefined(self.savedangles) && isDefined( origin ) && isDefined( angles ) )
...
endRound( reasonText, team )
{
	...
	else
	{
		...
		if( level.dvar["roundSound"] )
		{
			...
			hud = addTextHud( level, 5, 448, 1, "center", "top", 1.4 );
			                                 hud.horzalign = "center";
		                                     hud.vertalign = "top";
			hud setText( "^1>>^2Now playing: " + getDvar( "dr_song_" + song ) );
